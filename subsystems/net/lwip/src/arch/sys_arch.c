...
sys_thread_t sys_thread_new(const char *name, lwip_thread_fn thread, void *arg, int stacksize, int prio)
{
  ...
  int r0 = OSTaskCreateExt(thread, arg, sys_thread_stacks[sys_thread_num] - 1,
		  (uint8_t)prio, (uint16_t)prio, sys_thread_stacks[sys_thread_num] - 800, 800, 0, 3);
  if (r0 != 0)
  {
	  sys_thread_num--;
