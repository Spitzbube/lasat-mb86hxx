...
uint8_t MGC_DrcInit(MGC_Port *pPort)
{
...
#if 0
	console_send_string("MGC_DrcInit (todo.c): TODO\r\n");
#endif
...
#if 0
	{
		...
		sprintf(debug_string, "MGC_DrcInit: bEndCount=%u\r\n", pPort->bEndCount);
    ...
    if (pPort->pPrivateData != 0)
    {
                        ...
                        pPort->bEndCount, pPort->pPrivateData))
        {
            ...
            //°Ñendpoint0µÄÊý¾Ý¿½±´µ½localEndsËùÖ¸ÏòµÄ¶¯Ì¬ÄÚ´æ¿Õ¼ä,Æ«ÒÆÁ¿Îª
            // pArray->wItemCount * * pArray->wItemSize
            MUSB_ArrayAppend(&(pPort->LocalEnds), &end);
            //°ÑÆäËüendpointµÄÊý¾Ý¿½±´µ½localEndsËùÖ¸ÏòµÄ¶¯Ì¬ÄÚ´æ¿Õ¼ä
            /* add others */
