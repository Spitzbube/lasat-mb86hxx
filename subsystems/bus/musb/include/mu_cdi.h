...
typedef struct _MUSB_Device
{
    ...
    MUSB_DeviceDescriptor DeviceDescriptor; //0xc
    MUSB_ConfigurationDescriptor* pCurrentConfiguration; //0x20
    MUSB_ConfigurationDescriptor** apConfigDescriptors; //0x24
    uint8_t* pDescriptorBuffer;
    uint16_t wDescriptorBufferLength;
    MUSB_ConnectionSpeed ConnectionSpeed;
    uint8_t bBusAddress; //0x34
    struct _MUSB_Device* pParentUsbDevice; //0x38
    uint8_t bHubPort;
} MUSB_Device;
...
typedef struct _MUSB_ControlIrp
{
    ...
    uint8_t* pInBuffer; //0xc
    uint32_t dwInLength; //0x10
    uint32_t dwStatus; //0x14
    uint32_t dwActualOutLength;
    uint32_t dwActualInLength;
    MUSB_pfControlIrpComplete pfIrpComplete; //0x20
    void* pCompleteParam; //0x24
    uint8_t bAllowDma; //0x28
    //0x2c
} MUSB_ControlIrp;
...
typedef struct _MUSB_Irp
{
    ...
    uint8_t* pBuffer; //4
    uint32_t dwLength; //8
    uint32_t dwStatus; //0xc
    uint32_t dwActualLength;
    MUSB_pfIrpComplete pfIrpComplete; //0x14
    void* pCompleteParam; //0x18
    uint8_t bAllowShortTransfer; //0x1c
