...
void sub_234515f0(int r4, int r7)
{
	...
	if (r4 == 0)
	{
		...
		{
			//loc_23451634
			if (channel_database.Data_235441d8[1]/*favourites*/.arChannel[i] != 0xffff)
			{
				//0x23451644
				Data_2377d870[i].wData_0 =
						channel_database.arChannels[ channel_database.Data_235441d8[1]/*favourites*/.arChannel[i] ].service_id;

				Data_2377d870[i].wData_2 =
						channel_database.arTransponders[ channel_database.arChannels[ channel_database.Data_235441d8[1]/*favourites*/.arChannel[i] ].wTransponderIndex ].transport_stream_id;
			}
			//loc_23451680
		}
		//->loc_23451738
	...
	else
	{
		...
		for (i = 0; i < 8; i++)
		{
			...
			if (channel_database.Data_235441d8[1]/*favourites*/.arChannel[i] != 0xffff)
			{
				...
				for (j = 0; j < 6000; j++)
				{
					...
					if (r3 == 0xffff)
					{
						channel_database.Data_235441d8[1]/*favourites*/.arChannel[i] = 0xffff;
						//->loc_23451710
						break;
					}
					//0x234516c4
					...
					if (sl == r3)
					{
						...
						{
							//loc_234516fc
							channel_database.Data_235441d8[1]/*favourites*/.arChannel[i] = j;
							//->loc_23451710
							break;
						}
						//loc_23451704
		...
		//0x23451728
		channel_database.Data_23546118[1] = crc32((void*) &channel_database.Data_235441d8[1]/*favourites*/, 2000);
	}
	//loc_23451738
