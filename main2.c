
#include "data.h"


static uint8_t Data_23493c40[];
static uint8_t Data_23493d40[];
uint8_t* Data_23493840[256] = //23493840
{
	&Data_23493c40[0],
	&Data_23493d40[0],
};
uint8_t Data_23493c40[256] = //23493c40
{
	,
};
uint8_t Data_23493d40[256] = //23493d40
{
	,	
};


/* 23457bf0 - todo */
int sub_23457bf0(int r0, uint8_t* r1, int r2)
{
#if 0
	console_send_string("sub_23457bf0 (todo.c): TODO\r\n");
#endif

	int res;
	uint8_t ch;
	uint8_t lowByte = r0 & 0xff;
	uint8_t highByte = (r0 & 0xff00) >> 8;

	if (r2 != 0)
	{
		uint8_t* table = Data_23493840[highByte];
		if (table != 0)
		{
			ch = table[lowByte];
			if (ch != 0)
			{
				res = 1;
			}
			else
			{
				res = -1;
			}
		}
		else
		{
			res = -1;
		}
	}
	else
	{
		res = -1;
	}

	if (res == 1)
	{
		*r1 = ch;
	}

	return res;
}


